...
public class DELETE extends ConnectionHandler {
  ...
  public boolean sluttReg() {
    ...
    try {
      PreparedStatement loypeStmt = conn
          .prepareStatement("select sekvnr, postnr, tid from Reg where brikkenr=(?) order by sekvnr");
    ...
    // check that the runner has done the correct Posts (controls). Set status=dsq
    // in case of wrong controls
    ...
    try {
      PreparedStatement updStmt = conn
          .prepareStatement("update Loper set status='ok', lopstid=(?) where brikkenr= (?)");
